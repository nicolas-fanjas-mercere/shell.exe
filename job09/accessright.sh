...
for h in $(eval echo "{2..$i..1}");
	...
	case ${infos[5]} in
		sudo*)
			sudo useradd -G ftpgroup -m -u   ${infos[1]} -p ${infos[4]} usermod -aG sudo ${user[$h]}  
			echo "L'utilisateur ${user[$h]}a été crée et est passé sudo"
			;;
		*)
			sudo useradd -G ftpgroup -m -u ${infos[1]} -p ${infos[4]} ${user[$h]}  
